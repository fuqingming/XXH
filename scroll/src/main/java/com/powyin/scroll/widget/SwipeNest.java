...
public class SwipeNest extends ViewGroup implements NestedScrollingParent, ISwipe, ScrollingView {
    ...
    public void computeScroll() {
        ...
        if (mPreScrollY != scrollY) {
            ...
            // ----------------------------------------------------------------------------------------------------------------》》下拉
            if (0 == mPreScrollY && (mFreshStatus == FreshStatus.ERROR || mFreshStatus == FreshStatus.ERROR_NET
            ...
            if (0 > mPreScrollY) {                                                                                                                            //刷新下拉状态
                ...
                if (mFreshStatus == null) {
                    if (!mRefreshStatusContinueRunning) {
                        if (mPreScrollY < -mOverScrollTopMiddle) {
            ...
            case SUCCESS:                                                                                   //设置刷新成功 自动隐藏
                postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        mLoadedStatus = null;
                        mLoadedStatusContinueRunning = false;
                        mSwipeController.onSwipeStatue(SwipeController.SwipeStatus.SWIPE_HEAD_COMPLETE_OK, -getScrollY(), mOverScrollTop);
