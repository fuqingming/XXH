...
import com.jy.xxh.adapter.ChatAdapter;
...
public class ChatActivityc extends KJActivity implements PullLoadMoreRecyclerView.PullLoadMoreListener {
...
//        mRealListView.setSelector(android.R.color.transparent);
        ...
        Utils.setOnTouchEditTextOutSideHideIM(this,mRecyclerView);
...
//                .crop()
                ...
                .subscribe(new RxBusResultDisposable<ImageRadioResultEvent>() {
                    ...
                    protected void onEvent(ImageRadioResultEvent imageRadioResultEvent) throws Exception {
                        Uri dataUri = Utils.getMediaUriFromPath(ChatActivityc.this,imageRadioResultEvent.getResult().getOriginalPath());
                        if (dataUri != null) {
...
//        mPullLoadMoreRecyclerView.setFooterViewText("loading");
     ...
     */
    private ChatActivityc.OnChatItemClickListener getOnChatItemClickListener() {
        return new ChatActivityc.OnChatItemClickListener() {
            @Override
    ...
    class ChatRoomListener extends EaseChatRoomListener {
            ...
            runOnUiThread(new Runnable() {
                public void run() {
                    if (roomId.equals(m_strRoomeId)) {
                        Toast.makeText(ChatActivityc.this, com.hyphenate.easeui.R.string.the_current_chat_room_destroyed, Toast.LENGTH_LONG).show();
                        Activity activity = ChatActivityc.this;
        ...
        public void onRemovedFromChatRoom(final String roomId, final String roomName, final String participant) {
            runOnUiThread(new Runnable() {
                public void run() {
                    if (roomId.equals(m_strRoomeId)) {
                        Toast.makeText(ChatActivityc.this, com.hyphenate.easeui.R.string.quiting_the_chat_room, Toast.LENGTH_LONG).show();
                        Activity activity = ChatActivityc.this;
                        if (activity != null && !activity.isFinishing()) {
                            activity.finish();
                        }
...
//						Toast.makeText(getActivity(), "member exit:" + participant, Toast.LENGTH_LONG).show();
    ...
    protected void onChatRoomViewCreation() {
        final ProgressDialog pd = ProgressDialog.show(ChatActivityc.this, "", "Joining......");
        EMClient.getInstance().chatroomManager().joinChatRoom(m_strRoomeId, new EMValueCallBack<EMChatRoom>() {
            ...
            public void onSuccess(final EMChatRoom value) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if(ChatActivityc.this.isFinishing() || !m_strRoomeId.equals(value.getId()))
                            return;
                        pd.dismiss();
